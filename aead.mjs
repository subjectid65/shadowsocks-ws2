...
export class AEAD {
  ...
  decrypt(c, tag) {
    const d = createDecipheriv(this.algorithm, new Uint8Array(this.key), this.nonce, this.options)
    const m = []
  ...
  encrypt(m) {
    const e = createCipheriv(this.algorithm, new Uint8Array(this.key), this.nonce, this.options)
    const c = []
    c.push(e.update(m))
    c.push(e.final())
    c.push(e.getAuthTag())
